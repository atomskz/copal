<p align="right"><a href="../API.md">English</a> | <b>Русский</b></p>

# copal — Public API

Статус: **актуально** (соответствует публичным заголовкам `include/copal/*` по
состоянию на Этап 7). Версия: 1.0. Текущая версия библиотеки — **0.2.0**.
...
- Лог-колбэк — **единственный** механизм логирования (process-wide); получает
  ...
  шрифты). Без колбэка WARN/ERROR идут в stderr (только hosted).
- `cl_set_assert_handler` ставит хендлер на провал `CL_ASSERT` (debug-сборки).
  Hosted-дефолт логирует и делает abort; freestanding-embedder ставит свой, чтобы
  отрапортовать и остановиться (libc-assert нет). В release (NDEBUG) ассерты
  компилируются прочь.
...
/* Инжектируемый мьютекс для cross-thread task-очереди (cl_application_post).
 * NULL берёт hosted-дефолт (pthread / critical section); freestanding-сборка
 * дефолта не имеет и обязана инжектировать свой (на UEFI: RaiseTPL/RestoreTPL). */
...
typedef struct cl_application_desc {
    ...
    const cl_allocator_t *allocator;      /* NULL -> встроенный malloc (нет во freestanding) */
    ...
    const cl_mutex_iface_t *mutex;        /* NULL -> hosted-дефолт (см. выше) */
 ...
 * попапы складываются в стек (меню кладёт своё подменю сверху), клик в нижний
 * элемент схлопывает лежащие над ним. open_modal центрирует диалог и делает его
 * барьером: клики вне модала проглатываются, light-dismiss и замена работают
 * только НАД ним. Клик вне цепочки, либо close_popup(), закрывает её; закрытие
 * отложено (обработчик popup'а может сам запросить закрытие). Стек ограничен
 * (8 уровней); запрос сверх лимита игнорируется (WARN, CL_ERROR_UNSUPPORTED),
 * а popup уничтожается, а не течёт. */
...
enum cl_widget_flags {
    ...
    /* бит 3 зарезервирован (бывший DIRTY; не был реализован) */
    CL_WF_DEAD = 1u<<4,  /* внутренний: поддерево ждёт отложенного free (не трогать) */
...
struct cl_widget {
    ...
    float             flex;
    uint32_t          flags;
    uint32_t          cursor;   /* cl_cursor_t, показываемый при hover (widget.h) */
    void             *userdata;
...
Подменю поддержаны: пункт-подменю (`cl_menu_add_submenu`) открывает вложенное меню
рядом с собой по клику, Enter или Right, а попапы складываются в стек (§12). Пока
не поддержано: открытие нового popup из callback пункта (меню разрушается после
возврата callback).
