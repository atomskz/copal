<p align="right"><a href="../STRUCTURE.md">English</a> | <b>Русский</b></p>
...
│   │   ├── abi.c                  ABI-рукопожатие desc/ops (cl_abi_ok, cl_desc_fill)
...
│   ├── test_abi.c                 ABI-эволюция desc/ops (ADR-005): tail-толерантное рукопожатие
│   ├── test_soft.c                golden-пиксели software-рендерера (stub-fb, без SDL)
│   ├── test_gl_golden.c           GL-рендер против software-эталона (только SDL+GL; skip без GL)
│   ├── test_image.c               cl_image_create: валидация входа, отказ на overflow размера
...
│   ├── test_overlay.c             лимит overlay-стека (CL_WINDOW_MAX_OVERLAYS): отказ за пределом
...
│   ├── test_fuzz.c                робастность cl_font_load_memory/cl_image_create (фикс-seed, под ASan/UBSan)
│   ├── test_layout.c              flex/align/margin, вложенные боксы
│   ├── test_lifecycle.c           окно/фокус/hover/курсор, destroy из callback
│   ├── test_recursion.c           лимит глубины дерева (CL_WIDGET_MAX_DEPTH): без переполнения стека
│   └── test_gui.c                 интеграционный сценарий на mock-бэкендах
├── benchmarks/                    headless-бенчмарки (COPAL_BUILD_BENCHMARKS, по умолчанию OFF)
│   ├── CMakeLists.txt             цель copal_bench (линкуется с copal::mocks)
│   └── bench.c                    прогон render/layout через white-box-точки входа
...
| `src/core/foundation/abi.c` | ABI-рукопожатие desc/ops: `cl_abi_ok`, `cl_desc_fill` | нет |
...
| `src/render/soft/renderer_soft.c` | software/CPU растеризатор (SDF+AA, glyph-блит); без GL | нет |
...
Реализация:
  foundation (alloc/error/utf8/version/mutex/abi) → свои публичные .h + foundation_internal.h
...
cmake --build build
ctest --test-dir build --output-on-failure     # 24 сьюта + smoke-прогоны
...
`-DCOPAL_BUILD_BENCHMARKS=ON` (headless-бенчмарки; требует статической библиотеки),
