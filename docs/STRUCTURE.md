...
│   │   └── theme_internal.h
│   ├── render/
│   │   ├── renderer.h             внутренний интерфейс renderer (device/frame)
...
│   ├── platform/
│   │   ├── platform.h             интерфейс платформы, cl_pixmap, ops (incl. lock/unlock_framebuffer)
...
├── tests/
│   ├── CMakeLists.txt             регистрирует 14 CTest-сьютов
...
- **Внутренние** (`src/**/*_internal.h`, `src/platform/platform.h`,
  `src/render/renderer.h` и пр.) — не устанавливаются, без `CL_API`, доступны
  только внутри сборки библиотеки.
- Исключение: `widget_impl.h` — **устанавливается** (нужен авторам сторонних
  виджетов), помечен как «для расширения», раскрывает базу виджета (ARCHITECTURE §9).
  Зонтичный `copal.h` его НЕ включает — приложению он не нужен.
