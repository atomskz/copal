...
(платформенная операция `present_region`). Полная перерисовка остаётся для
...
сверяет их и при расхождении → `NULL` + `CL_ERROR_ABI_MISMATCH`. Рост базы/vtable
требует перекомпиляции приложения.
