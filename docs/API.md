...
Правила: `cl_widget_destroy()` уничтожает **немедленно** — отложенной
DEAD-очереди нет (ARCHITECTURE §5). Уничтожать виджет из его собственного
callback можно, только если обработчик после этого вернёт `true` (остановит
всплытие по уже освобождённой цепочке); иначе отложите уничтожение через
`cl_application_post()`. Встроенные виджеты вызывают пользовательский callback
последним действием. `void *user` — «слабый»: библиотека им не владеет.
...
- `cl_widget_destroy(w)` — уничтожает поддерево **немедленно** (в том числе
  внутри dispatch — правила безопасности см. §6).
...
/* Overlay-popup (единственный слой поверх content). open_popup показывает `popup`
 * в позиции окна `at` (клампится по экрану) и берёт его во владение. Клик вне,
 * либо close_popup(), закрывает; активен только один popup; закрытие отложено
